...
public class Sound : IAddressSpace
{
	...
	public void Tick()
	{
		...
		{
			SoundModeBase abstractSoundMode = _allModes[i];
	...
	{
		foreach (var soundMode in _allModes)
		{
			if (soundMode.Accepts(address))
			{
				return soundMode;
			}
		}
	...
	{
		var space = GetAddressSpace(address)
			?? throw new ArgumentException("Invlaid byte address");
